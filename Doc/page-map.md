...
| /admin | Admin panel | users R, tunnels R, tokens R, payments R | GET/PUT/DELETE /users, /tokens/admin/*, /payments/admin/*, /ip-monitor/*, GET /tunnels/stats | admin.html | JWT, role=admin | /dashboard (via user link), /docs, /redoc, /health | /login (on admin login) | users/tunnels/tokens/payments + Redis IP stats — ALL admin views |
| /dashboard | User dashboard | users R/W (custom-domain, plan), tokens R/W, tunnels R | GET /auth/me, /auth/tunnel-token, POST /auth/regenerate-token, /tokens CRUD, PUT /users/me/custom-domain, POST /payments/checkout | dashboard.html | JWT | / (logout), /login (expired) | / (nav when logged in), /admin | users, tokens, tunnels, payments |
